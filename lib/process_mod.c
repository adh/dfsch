...
dfsch_object_t* dfsch_module_process_register(dfsch_object_t *ctx){
  dfsch_defcanon_cstr(ctx, "<process-port>", 
                    DFSCH_PROCESS_PORT_TYPE);
  dfsch_defcanon_cstr(ctx, "<process-input-port>", 
                    DFSCH_PROCESS_INPUT_PORT_TYPE);
  dfsch_defcanon_cstr(ctx, "<process-output-port>", 
                    DFSCH_PROCESS_OUTPUT_PORT_TYPE);

  dfsch_defcanon_cstr(ctx, "spawn-process!",
                    DFSCH_PRIMITIVE_REF(spawn));
  dfsch_defcanon_cstr(ctx, "spawn-process-with-input-port!",
                    DFSCH_PRIMITIVE_REF(spawn_with_input_port));
  dfsch_defcanon_cstr(ctx, "spawn-process-with-output-port!",
                    DFSCH_PRIMITIVE_REF(spawn_with_output_port));
  dfsch_defcanon_cstr(ctx, "close-process-port!",

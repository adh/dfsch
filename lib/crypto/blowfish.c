...
*/
static void blowfish_encrypt(blowfish_key_t* key, 
...
static void blowfish_setup(blowfish_key_t* ctx, 
                           const unsigned char *key, int keylen)
{
   ...
   if (keylen < 8 || keylen > 56) {
     dfsch_error("Invalid key length (must be between 8 and 56 bytes)",
                 DFSCH_MAKE_FIXNUM(keylen));
   }
   ...
   for (x = 0; x < 18; x += 2) {
       /* encrypt it */
     blowfish_encrypt(ctx, B, B);
       /* copy it */
       LOAD32H(ctx->K[x], &B[0]);
       LOAD32H(ctx->K[x+1], &B[4]);
   }
   ...
   for (x = 0; x < 4; x++) {
       for (y = 0; y < 256; y += 2) {
         ...
         blowfish_encrypt(ctx, B, B);
          /* copy it */
          LOAD32H(ctx->S[x][y], &B[0]);
          LOAD32H(ctx->S[x][y+1], &B[4]);
       }
   }
}

static void blowfish_decrypt(blowfish_key_t* key,
...
dfsch_block_cipher_t dfsch_crypto_blowfish_cipher = {
  .type = {
    .type = DFSCH_BLOCK_CIPHER_TYPE,
    .name = "crypto:blowfish",
    .size = sizeof(blowfish_key_t)
  },
